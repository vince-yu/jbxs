...
@interface SDGoodDetailViewController ()
@property (nonatomic ,strong) SDGoodBarView *barView;
@end

@implementation SDGoodDetailViewController
...
- (UIView *)barView{
    if (!_barView) {
        SDGoodBarStyle type = SDGoodBarStyleCart;
        if (self.goodModel.type.integerValue == SDGoodTypeNamoal) {
            type = SDGoodBarStyleBuyAndCart;
        }
...
- (SDDetailTableView *)tableView{
    if (!_tableView) {
        _tableView = [[SDGoodDetailTableView alloc] initWithFrame:CGRectZero style:UITableViewStylePlain goodModel:self.goodModel];
