...
//        for (NSInteger i = 0; i < _numberOfNodes ; i++)
        NSInteger count = _numberOfNodes;
        if (!self.fullNode) {
            count = _numberOfNodes - 1;
        }
...
//                [[UIColor colorWithRed:180.0/255.0 green:30.0/255.0 blue:39.0/255.0 alpha:1.0] set];
            ...
            //3、添加新的小node
            if (i < _numberOfHighlightNodes) {
                UIBezierPath *centerPoint = [UIBezierPath bezierPathWithRoundedRect:CGRectMake(i *(space + h)+ x + h / 4, y + h/ 4, h/2, h/2) cornerRadius:h *0.5/2];
                [[UIColor whiteColor] set];
                [centerPoint fill];
                [nodes appendPath:centerPoint];
            }
        }
    }
